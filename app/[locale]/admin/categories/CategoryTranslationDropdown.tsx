...
interface CategoryTranslationDropdownProps {
...
export default function CategoryTranslationDropdown({
  availableLanguages,
  selectedLanguages,
  activeLanguage,
  currentTranslation,
  onTranslate,
  className,
}: CategoryTranslationDropdownProps) {
  ...
  const handleTranslate = async () => {
    ...
    if (!trimmedTitle) {
      toast.error(
        "Please fill in the title for the current language before translating"
      );
      return;
    }
    ...
    try {
      ...
      const titleTranslations = results[0];
      const descriptionTranslations = results[1] || {};
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          disabled={!currentTranslation.title.trim()}
        >

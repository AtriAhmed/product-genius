  ...
  if (paymentMethods.length === 0) {
    return (
      <div className="space-y-3 py-8 border-2 border-slate-300 dark:border-slate-700 border-dashed rounded-xl bg-gradient-to-br from-white dark:from-slate-800 to-slate-100 dark:to-slate-900 text-center">
        ...
        <div>
          <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base">No payment methods found</h3>
          <p className="mt-1 text-slate-600 dark:text-slate-400 text-xs">Add a payment method to continue</p>
        </div>
        <Button
      ...
      >
        {loading ? (
          ...
          <>
            ...
            {t("pay")} {formatCurrency((invoice.amountCents || 0) / 100)}
          </>
        )}
      </Button>
    </div>

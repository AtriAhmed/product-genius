...
import CheckoutReviewStep from "@/app/[locale]/dashboard/orders/cart/CheckoutReviewStep";
import CheckoutPaymentStep from "@/app/[locale]/dashboard/orders/cart/CheckoutPaymentStep";
...
export default function CheckoutDialog({
  ...
  const fetchPaymentMethods = async () => {
    ...
    try {
      ...
      setError(tPricing("failed to load payment methods"));
    } finally {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        deliveryAddress1: deliveryInfo.deliveryAddress1,
        deliveryAddress2: deliveryInfo.deliveryAddress2,
        deliveryCity: deliveryInfo.deliveryCity,
        deliveryState: deliveryInfo.deliveryState,
        deliveryZip: deliveryInfo.deliveryZip,
        deliveryCountry: deliveryInfo.deliveryCountry,
      };
      ...
      if (response.data.requiresAction) {
        ...
        // This would need additional implementation with Stripe Elements
        setError(
      ...
      setError(error.response?.data?.error || t("failed to create order"));
      setStep("payment");
    } finally {
      setLoading(false);
    }
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex-1 text-center">
                <DialogTitle className="flex justify-center items-center gap-2 bg-clip-text bg-gradient-to-r from-primary-600 dark:from-primary-400 via-primary-600 dark:via-primary-400 to-primary-700 dark:to-primary-500 font-bold text-transparent text-lg">
                  ...
                  {step === "processing" && (
                    <>
                      <CreditCard className="w-5 h-5 text-yellow-500 animate-pulse" />
                      {tPricing("processing")}
                    </>
                  )}
                </DialogTitle>
              ...
              <div className="space-y-4 py-8 text-center">
                ...
                <div>
                  ...
                  <p className="mt-1 text-neutral-600 dark:text-neutral-400 text-xs">
                    {tPricing("please don't close this window")}
                  </p>
                </div>
              </div>
            )}
